...
def get_test_type_info(test_type):
    # Diagnostyka: Wyświetlenie wszystkich rekordów w tabeli TEST_TYPES
    all_types = session.query(TestType).all()
    print("Available test types in TEST_TYPES table:")
    for t in all_types:
        print(f"- {t.test_type}")

    # Próbujemy znaleźć rekord – ignorujemy wielkość liter
...
# Wczytanie danych Fashion MNIST z CSV
train_df = pd.read_csv("../../Algorytmy/datasets/fashion-mnist_train.csv")
test_df = pd.read_csv("../../Algorytmy/datasets/fashion-mnist_test.csv")

# Oddzielenie etykiet i pikseli
y_train = train_df['label'].values
x_train = train_df.drop(columns=['label']).values.astype('float32') / 255.0
y_test = test_df['label'].values
x_test = test_df.drop(columns=['label']).values.astype('float32') / 255.0

# Test Functional na Fashion MNIST
...
THRESHOLD = 0.85  # Próg dla Functional Test – oczekujemy accuracy >= 85% (Fashion MNIST jest trudniejszy niż MNIST)
...
if not test:
    test = Test(
        issue_key=ISSUE_KEY,
        test_name="WYM-001 Functional Fashion 01",
...
# Raport do JIRA
test_type_info = get_test_type_info("Funkcjonalne")
if test_accuracy >= THRESHOLD:
    report_success_to_jira(ISSUE_KEY, test_accuracy, "Functional (Fashion MNIST)", test_type_info)
else:
    ...
    if defect_key:
        defect = Defect(
            ...
            description=f"Functional Test (Fashion MNIST): Accuracy below threshold: {test_accuracy:.4f}",
            precision=precision,
            recall=recall,
            f1_score=f1,
