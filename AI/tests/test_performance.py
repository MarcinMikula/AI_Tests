...
THRESHOLD = 0.95  # Próg dla Performance Test – oczekujemy accuracy, precision, recall, f1 >= 95%
...
if not test:
    test = Test(
        issue_key=ISSUE_KEY,
        test_name="WYM-002 Performance 01",
...
result = TestResult(
    ...
    status="Passed" if all([test_accuracy >= THRESHOLD, precision >= THRESHOLD, recall >= THRESHOLD, f1 >= THRESHOLD]) else "Failed"
)
session.add(result)
session.commit()
...
# Raport do JIRA
test_type_info = get_test_type_info("Wydajności")
if all([test_accuracy >= THRESHOLD, precision >= THRESHOLD, recall >= THRESHOLD, f1 >= THRESHOLD]):
    report_success_to_jira(ISSUE_KEY, test_accuracy, "Performance", test_type_info)
else:
    ...
    if defect_key:
        defect = Defect(
            ...
            description=f"Performance Test: Accuracy={test_accuracy:.4f}, Precision={precision:.4f}, Recall={recall:.4f}, F1={f1:.4f} (Threshold: {THRESHOLD})",
            precision=precision,
            recall=recall,
            f1_score=f1,
