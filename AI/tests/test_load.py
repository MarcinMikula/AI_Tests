...
execution_time_training = time.time() - start_time

# Pomiar czasu predykcji dla 10 000 obrazów
start_time = time.time()
y_pred = np.argmax(model.predict(x_test), axis=1)
execution_time_prediction = time.time() - start_time

# Obliczenie średniego czasu na obraz
num_images = len(x_test)  # 10 000 w MNIST
avg_time_per_image = execution_time_prediction / num_images

# Test na danych oryginalnych
test_loss, test_accuracy = model.evaluate(x_test, y_test, verbose=0)

THRESHOLD = 0.01  # Próg dla Load Test – oczekujemy średniego czasu na obraz < 0.01 sekundy
...
result = TestResult(
    ...
    execution_time=execution_time_prediction,
    model_details=model_details,
    class_distribution=class_dist,
    status="Passed" if avg_time_per_image <= THRESHOLD else "Failed"
)
session.add(result)
session.commit()
...
# Raport do JIRA
test_type_info = get_test_type_info("Obciążeniowe")
if avg_time_per_image <= THRESHOLD:
    report_success_to_jira(ISSUE_KEY, test_accuracy, "Load", test_type_info)
else:
    defect_key, precision, recall, f1, training_details, environment, top_misclassification = report_defect_to_jira(
        result.id, test_accuracy, THRESHOLD, ISSUE_KEY, y_test, y_pred, model, EPOCHS, TEST_SIZE, execution_time_prediction, model_details, class_dist
    )
    if defect_key:
        defect = Defect(
            test_result_id=result.id,
            issue_key=defect_key,
            description=f"Load Test: Average time per image {avg_time_per_image:.6f} seconds (Threshold: {THRESHOLD})",
            precision=precision,
            recall=recall,
            f1_score=f1,
