...
THRESHOLD = 0.95  # Próg dla Functional Test – oczekujemy accuracy >= 95%
...
if not test:
    test = Test(
        issue_key=ISSUE_KEY,
        test_name="WYM-001 Functional 01",
...
if test_accuracy >= THRESHOLD:
    report_success_to_jira(ISSUE_KEY, test_accuracy, "Functional", test_type_info)
else:
    ...
    if defect_key:
        defect = Defect(
            ...
            description=f"Functional Test: Accuracy below threshold: {test_accuracy:.4f}",
            precision=precision,
            recall=recall,
            f1_score=f1,
